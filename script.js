...
const numero=document.getElementById("certificat").value.trim().toUpperCase();

const resultat=document.getElementById("resultat");

const cert=certificats.find(c=>c.numero===numero);
if(cert){
resultat.style.display="inline-block";
resultat.innerHTML=`
<h3>✅ Certificat authentique</h3>
<p><strong>Nom :</strong> ${cert.nom}</p>
<p><strong>Formation :</strong> ${cert.formation}</p>
<p><strong>Session :</strong> ${cert.session}</p>
<p><strong>Durée :</strong> ${cert.duree}</p>
<p><strong>Date :</strong> ${cert.date}</p>
<p><strong>N° :</strong> ${cert.numero}</p>
`;
}
else{
resultat.style.display="inline-block";
resultat.innerHTML=`
<h3 style="color:red;">❌ Certificat introuvable</h3>
<p>Aucun certificat ne correspond à ce numéro.</p>
